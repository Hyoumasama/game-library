...
The twelve requested historical release/package groups now have separate identities and specific relationships. The eight redundant relationship classifications were corrected; dated Stellar Blade demo copies are mapped by year. Every mapping/edge change, before/after counts, preservation checks and final test results are in [RELATIONSHIP_CLEANUP_REPORT.md](RELATIONSHIP_CLEANUP_REPORT.md). All 2,234 ownership rows remain unchanged; pending/suspicious/conflicting/orphan/broken-FK counts are zero. The latest suite has 46 unit tests. Enrichment preserves existing verified facts rather than automatically removing/downgrading them.
